...
class SaleOrderInherit(models.Model):
    ...
    def action_send_zalo_message(self):
        recipient_ids = [(6, 0, self.partner_id.ids)]
        name = self.name
        message = 'Hello ' + str(self.partner_id.name) + '. Your order ' + str(self.name) + ' amounting in $' + str(
            self.amount_total) + ' has been confirmed. Thank you for your trust!'

        ctx = {
            'default_recipient_ids': recipient_ids,
            'default_source_origin': name,
