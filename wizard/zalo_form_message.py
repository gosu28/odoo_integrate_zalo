...
class ZaloFormMessage(models.TransientModel):
    ...
    def action_send(self):
        ...
        if self._context['default_model'] == 'sale.order':
            so_message = self.env['sale.order'].sudo().search([('name', '=', self._context['default_source_origin'])])
            ...
            po_message = self.env['purchase.order'].sudo().search(
                [('name', '=', self._context['default_source_origin'])])
